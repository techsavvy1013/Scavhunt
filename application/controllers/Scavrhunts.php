...
class Scavrhunts extends BaseController
{
    ...
    public function editHunt()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $doorImagePath = $this->security->xss_clean($this->input->post('doorImagePath'));
            $moveOnWrong = $this->security->xss_clean($this->input->post('moveOnWrong'));
    ...
    public function saveChallengePoints()
    {
        ...
        $challenges = $this->challenge_model->getChallengesByHunt($result->hunt_id);
        $i = 0;
        for ($i = 0; $i < count($challenges); $i++) {
            if ($challenges[$i]->id == $result->chg_id)
                break;
        }
        $msg = "You got " . $points . " points for Challenge " . ($i + 1);
        $this->notification_model->insertNotification(
            $result->player_id,
            $msg
        );
        // echo "success";
